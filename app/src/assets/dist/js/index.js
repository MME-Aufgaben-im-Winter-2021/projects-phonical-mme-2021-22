          ...
          "CLASS": function(className) {
            ...
            return pattern ||
              ...
              classCache(
                ...
                function(elem) {
                  return pattern.test(
                    typeof elem.className === "string" && elem
                    .className ||
          ...
          pipe: function( /* fnDone, fnFail, fnProgress */ ) {
            ...
            return jQuery.Deferred(function(newDefer) {
              jQuery.each(tuples, function(_i, tuple) {
                ...
                var fn = isFunction(fns[tuple[4]]) && fns[
                  tuple[4]];
      ...
      reliableTrDimensions: function() {
        ...
        if (reliableTrDimensionsVal == null) {
          ...
          reliableTrDimensionsVal = (parseInt(trStyle.height, 10) +
              ...
              parseInt(trStyle.borderBottomWidth, 10)) === tr
            .offsetHeight;
    ...
    jQuery.cssHooks[dimension] = {
      ...
      set: function(elem, value, extra) {
        ...
        if (isBorderBox && scrollboxSizeBuggy) {
          subtract -= Math.ceil(
            elem["offset" + dimension[0].toUpperCase() + dimension
              .slice(1)] -
  ...
  jQuery.fn.extend({
    ...
    position: function() {
      ...
      if (jQuery.css(elem, "position") === "fixed") {
        ...
        while (offsetParent &&
          (offsetParent === doc.body || offsetParent === doc
            .documentElement) &&
...
let api = {
	...
	listDocuments: (collectionId, quries=[], limit=100) => {
		return api.provider().database.listDocuments(collectionId, quries, limit)
	},
...
async function search_user() {
	...
	for (const row of profiles.documents) {
		if (row.user_email && search_txt && row.user_email.includes(search_txt) && row.user_id != cur_user) {
			if(available_users.includes(row.user_id)){
				...
				for (const room of room_users.documents) {
					const rooms_matched_with_search = window.contacts.filter((r) => r.$id == room.room_id)
	...
	for (const room_user of room_users.documents) {
		if (room_user.room_id == contact_id) {
			...
			if(room.room_owner_id == cur_user && room_user.user_id != cur_user){
				delete_action = `<a class="btn btn-icon" onclick="delete_chat('${room_user.user_id}')"><i class="fa fa-trash"></i></a>`
			}
